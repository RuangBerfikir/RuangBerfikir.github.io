...
function validateMaterial(judul, deskripsi, link, video = '') {
  const links = normalizeMaterialLinks(link);
  const videoLinks = normalizeMaterialLinks(video);
  ...
  const invalidVideo = videoLinks.find(item => !isValidUrl(item));
  if (invalidVideo) {
    showToast('Format link video tidak valid. Gunakan URL video yang benar.', 'error');
    return false;
  }
  return true;
}
...
const fallbackKuis = {
  KJR: [
    {
      soal: 'Tujuan utama dari konsep confidentiality dalam keamanan jaringan adalah …',
      opsi: ['Mempercepat transfer data', 'Menyimpan data dalam jumlah besar', 'Menjaga data agar tetap tersedia', 'Menjaga kerahasiaan informasi', 'Memperbaiki jaringan rusak'],
      jawaban: 3
    },
    {
      soal: 'Yang dimaksud dengan kerahasiaan (confidentiality) dalam keamanan jaringan adalah …',
      opsi: ['Memastikan data selalu tersedia', 'Memastikan data akurat', 'Mencegah pihak ketiga mengakses data sensitif', 'Mencegah serangan DDoS', 'Memastikan integritas data'],
      jawaban: 2
    },
    {
      soal: 'Di bawah yang merupakan ancaman fisik …',
      opsi: ['Korsleting', 'Virus', 'DDoS', 'Traffic flooding', 'SQL Injection'],
      jawaban: 0
    },
    {
      soal: 'Sistem keamanan yang membagi jaringan ke dalam beberapa segmen, dimana setiap segmen bekerja sebagai pemilik jaringan sendiri disebut …',
      opsi: ['Website security', 'VPN', 'Cloud Security', 'Application Security', 'Network Segmentation'],
      jawaban: 4
    },
    {
      soal: 'Ancaman yang diincar dari lokasi yang jauh terhadap jaringan, dimana penyerang tidak memiliki akses langsung ke perangkat target …',
      opsi: ['Sniffer', 'Hacking', 'Remote attack', 'Hole', 'Phreaking'],
      jawaban: 2
    },
    {
      soal: 'VPN bekerja dengan …',
      opsi: ['Mengenkripsi koneksi internet', 'Menghapus history browsing', 'Mengganti alamat IP permanen', 'Menonaktifkan cookies', 'Mengatur firewall otomatis'],
      jawaban: 0
    },
    {
      soal: 'Sistem untuk membatasi akses ke layanan administratif pada router adalah …',
      opsi: ['Telnet', 'SSH', 'RBAC', 'ACLs', 'AAA'],
      jawaban: 3
    },
    {
      soal: 'Yang dimaksud dengan access control pada keamanan jaringan adalah …',
      opsi: ['Mengamankan akses ke perangkat keras komputer', 'Menerapkan kebijakan keamanan pada aplikasi', 'Memindai email untuk malware', 'Membatasi jumlah pengguna untuk mengakses jaringan komputer', 'Memeriksa aplikasi untuk keamanan'],
      jawaban: 3
    },
    {
      soal: 'Control keamanan yang melibatkan penggunaan kunci autentikasi biometric termasuk dalam jenis …',
      opsi: ['Keamanan teknis', 'Keamanan administratif', 'Keamanan jaringan public', 'Keamanan jaringan fisik', 'Keamanan cloud'],
      jawaban: 0
    },
    {
      soal: 'Cara mencegah serangan jaringan yang digunakan untuk mendeteksi dan merespons ancaman paling berbahaya sekalipun masuk ke dalam jaringan yaitu dengan cara …',
      opsi: ['Melakukan segmentasi jaringan', 'Menerapkan Next Generation Firewall (NGFW)', 'Menggunakan Network Address Translation (NAT)', 'Menyebarkan umpan jaringan', 'Memanfaatkan network analytics'],
      jawaban: 1
    },
    {
      soal: 'Penyerang yang hanya memantau data tanpa mengubahnya melakukan serangan …',
      opsi: ['Aktif', 'Fisik', 'Pasif', 'DDoS', 'Internal'],
      jawaban: 2
    },
    {
      soal: 'Salah satu serangan DDoS terbesar yang terjadi pada tahun 2020 menargetkan …',
      opsi: ['Facebook', 'Google', 'Netflix', 'IBM Cloud', 'AWS (Amazon Web Service)'],
      jawaban: 4
    },
    {
      soal: 'Jenis-jenis keamanan jaringan yang umum digunakan sebagai berikut, kecuali …',
      opsi: ['Data control', 'Email security', 'Firewall', 'Antivirus', 'Network segmentation'],
      jawaban: 0
    },
    {
      soal: 'Untuk mencegah serangan keamanan jaringan pada sebuah aplikasi yang dibuat karena kerentannya terhadap sebuah serangan maka digunakan keamanan jaringan …',
      opsi: ['Application security', 'Access control', 'Email security', 'Cloud security', 'Message security'],
      jawaban: 0
    },
    {
      soal: 'Jenis malware yang munculkan iklan-iklan yang tidak diinginkan pada sebuah perangkat adalah …',
      opsi: ['Worm', 'Spyware', 'Adware', 'Virus', 'Trojan'],
      jawaban: 2
    },
    {
      soal: 'Alasan mitigasi ancaman perlu diperbarui secara berkala karena …',
      opsi: ['Serangan siber hanya terjadi sekali dalam seumur hidup', 'Resiko keamanan tidak pernah berubah', 'Ancaman keamanan selalu berkembang dan berubah', 'Tindakan mitigasi tidak pernah berhasil', 'Semua perangkat keras selalu aman dari kerentanan'],
      jawaban: 2
    },
    {
      soal: 'Sebuah perangkat keras atau perangkat lunak jahat yang dirancang untuk memodifikasi paket data yang dikirimkan, seperti mengubah alamat sumber tujuan, mengubah data dalam paket, atau menambahkan data palsu ke dalam paket disebut …',
      opsi: ['Packet injector', 'Brute force attack', 'Phishing kit', 'Sniffer', 'Exploit kit'],
      jawaban: 0
    }
  ],
  PKPJ: [
  ...
  TJKDN: [
    {
      soal: 'Alat yang digunakan untuk menghubungkan computer ke jaringan kabel adalah …',
      opsi: ['Modem', 'Kabel ethernet', 'Hub', 'Access point', 'Router'],
      jawaban: 1
    },
    {
      soal: 'Untuk menghubungkan jaringan tipe kabel atau topologi serta segmentasi IP yang berbeda, maka komponen yang harus digunakan adalah …',
      opsi: ['Router', 'Repeater', 'Switch', 'Bridge', 'Hub'],
      jawaban: 0
    },
    {
      soal: 'Terdapat beberapa jenis topologi yang digunakan untuk jaringan computer, topologi jaringan yang paling umum digunakan dalam jaringan rumah adalah …',
      opsi: ['Bus', 'Mesh', 'Star', 'Ring', 'Tree'],
      jawaban: 2
    },
    {
      soal: 'Topologi yang sering digunakan dalam kondisi dimana tidak boleh ada hubungan komunikasi yang terputus di antara node dalam jaringan komputer tersebut …',
      opsi: ['Topologi mesh', 'Topologi tree', 'Topologi star', 'Topologi ring', 'Topologi bus'],
      jawaban: 0
    },
    {
      soal: 'Salah satu penyebab utama gangguan sinyal pada jaringan nirkabel adalah …',
      opsi: ['Interferensi dari perangkat lain', 'Router yang terlalu dekat', 'Kabel yang buruk', 'Pengaturan IP yang salah', 'Keterbatasan konsentrator'],
      jawaban: 0
    },
    {
      soal: 'Jaringan komputer yang digunakan untuk komunikasi antara komputer dalam sebuah gedung, sekolah, atau kantor adalah …',
      opsi: ['PAN', 'MAN', 'LAN', 'Wireless', 'Wired'],
      jawaban: 2
    },
    {
      soal: 'Berikut bukan kelebihan topologi ring adalah …',
      opsi: ['Hemat kabel', 'Dapat melayani lalu lintas yang tepat', 'Tidak membutuhkan hub', 'Pengiriman tidak menunggu giliran', 'Penataan kabel lebih sederhana'],
      jawaban: 3
    },
    {
      soal: 'Manfaat melakukan enkapsulasi pada jaringan adalah …',
      opsi: ['Paket data tidak tersampaikan', 'Data menjadi memiliki identitas', 'Menghambat pengiriman data', 'Mempercepat proses pengiriman paket data', 'Mengurangi terjadinya error'],
      jawaban: 1
    },
    {
      soal: 'LAN yang sudah topologi star dengan memakai kabel UTP adalah …',
      opsi: ['Router', 'Switch', 'Bridge', 'Hub', 'Repeater'],
      jawaban: 1
    },
    {
      soal: 'Salah satu jenis koneksi jaringan WAN dengan karakteristik komputer pengiriman mentransmisikan data melalui koneksi yang sudah terbentuk disebut …',
      opsi: ['Leased line', 'Packet switched', 'Circuit switched', 'Point to point', 'Synchronous serial'],
      jawaban: 2
    },
    {
      soal: 'Berikut ini yang merupakan karakteristik jaringan WAN, kecuali …',
      opsi: ['Menggunakan koneksi serial', 'Menggunakan jalur layanan umum', 'Melakukan pertukaran paket data dan frame di router dan switch', 'Beroperasi pada physical layer dan transport layer', 'Terhubung ke perangkat yang tersebar ke area yang bagus'],
      jawaban: 3
    },
    {
      soal: 'Langkah pertama yang dilakukan saat melakukan troubleshooting jaringan adalah …',
      opsi: ['Restart semua perangkat', 'Mengganti semua kabel', 'Memeriksa koneksi fisik', 'Menghubungi ISP', 'Menguji koneksi'],
      jawaban: 2
    },
    {
      soal: 'Salah satu tujuan utama dari pemeliharaan rutin …',
      opsi: ['Meningkatkan kecepatan internet', 'Mengganti perangkat lama', 'Menambah biaya operasional', 'Mencegah kerusakan dan downtime', 'Meningkatkan keamanan jaringan'],
      jawaban: 3
    },
    {
      soal: 'Jenis kabel yang memiliki dua konduktor yang dapat digunakan dan merupakan kabel pasangan berpilin adalah …',
      opsi: ['Koaksial', 'Fiber optic', 'Twisted pair', 'Ethernet', 'Aux'],
      jawaban: 2
    },
    {
      soal: 'Jenis virus trojan yang dirancang untuk mendapatkan akses dan kendali jarak jauh, atau serang adalah …',
      opsi: ['Backdoor trojan', 'Downloader trojan', 'Root trojan', 'DDoS attack trojan', 'Fake antivirus'],
      jawaban: 0
    },
    {
      soal: 'Kondisi dimana system tidak dapat bekerja secara optimal karena adanya komponen komputer yang kurang sesuai, kondisi berikut disebut …',
      opsi: ['Botnet', 'Bottleneck', 'Collision', 'Serangan virus trojan', 'Device kentang'],
      jawaban: 1
    },
    {
      soal: 'Program berbahaya yang menyamar sebagai software yang valid disebut …',
      opsi: ['Botnet', 'Bottleneck', 'Collision', 'Serangan virus trojan', 'Device kentang'],
      jawaban: 3
    }
  ]
...
let quizLockActive = false;
let quizLockWarningShown = false;
let quizAutoClosed = false;
const QUIZ_LOCK_TITLE = document.title || 'RuangKelas TJKT';
const QUIZ_VIOLATION_KEY = 'ruangkelas.quizViolations';
const mapelNames = { KJR: 'Keamanan', PKPJ: 'Pemasangan perangkat', TJKDN: 'Kabel dan nirkabel' };

function recordQuizViolation(reason) {
  const nama = document.getElementById('nama-siswa')?.value.trim() || 'Tanpa nama';
  const kelas = document.getElementById('kelas-siswa')?.value || 'Belum dipilih';
  const mapel = document.getElementById('select-mapel')?.value || 'KJR';
  const entry = {
    nama,
    kelas,
    mapel,
  ...
  try {
    const existing = JSON.parse(localStorage.getItem(QUIZ_VIOLATION_KEY) || '[]');
    const next = [...existing, entry].slice(-50);
    localStorage.setItem(QUIZ_VIOLATION_KEY, JSON.stringify(next));
  } catch (err) {}
}

function enforceQuizLock() {
  if (!quizLockActive || quizAutoClosed) return;

  const overlay = document.getElementById('quiz-lock-overlay');
  const shouldLock = document.visibilityState === 'hidden' || !document.hasFocus();

  if (shouldLock) {
    if (overlay) overlay.classList.remove('hidden');
    document.title = 'Kuis aktif — jangan buka tab lain';
    forceFinishQuiz('Tab lain dibuka');
    return;
  }

  if (overlay) overlay.classList.add('hidden');
  document.title = QUIZ_LOCK_TITLE;
}

function activateQuizLock() {
  if (quizLockActive) return;
  quizLockActive = true;
  quizAutoClosed = false;
  quizLockWarningShown = false;

  const overlay = document.getElementById('quiz-lock-overlay');
  if (overlay) overlay.classList.remove('hidden');

  document.addEventListener('visibilitychange', enforceQuizLock);
  window.addEventListener('blur', enforceQuizLock);
  window.addEventListener('focus', enforceQuizLock);
  window.addEventListener('pagehide', handleQuizPageLeave);
  window.addEventListener('beforeunload', handleQuizPageLeave);
...
function deactivateQuizLock() {
  quizLockActive = false;
  quizAutoClosed = false;
  quizLockWarningShown = false;
  ...
  document.removeEventListener('visibilitychange', enforceQuizLock);
  window.removeEventListener('blur', enforceQuizLock);
  window.removeEventListener('focus', enforceQuizLock);
  window.removeEventListener('pagehide', handleQuizPageLeave);
  window.removeEventListener('beforeunload', handleQuizPageLeave);
...
function handleQuizPageLeave() {
  if (quizLockActive && !quizAutoClosed) {
    forceFinishQuiz('Halaman ditutup');
  }
}

function forceFinishQuiz(reason = 'Halaman berpindah') {
  if (!currentKuisData.length || quizAutoClosed) return;

  quizAutoClosed = true;
  clearInterval(timerId);
  saveSelectedAnswer();
  const hasil = document.getElementById('hasil-kuis');
  const finalScore = currentKuisData.reduce((total, question, index) => total + (selectedAnswers[index] === Number(question.jawaban) ? 1 : 0), 0);
  score = finalScore;

  if (hasil) {
    hasil.className = 'success';
    hasil.textContent = `Kuis otomatis ditutup karena ${reason.toLowerCase()}. Skormu ${score} dari ${currentKuisData.length}! Jawaban terkirim.`;
  }

  recordQuizViolation(reason);
  ...
  const opsiBox = document.getElementById('opsi-box');
  if (soalBox) soalBox.textContent = `Kuis ditutup otomatis karena ${reason.toLowerCase()}.`;
  if (opsiBox) opsiBox.innerHTML = '';
...
function getMaterialVideos(material) {
  const videos = normalizeMaterialLinks(material.video);
...
function renderMateri() {
  ...
  document.getElementById('materi-container').innerHTML = items.length ? items.map(m => { 
    ...
    const videoLinks = getMaterialVideos(m);
    ...
    const videosHtml = videoLinks.map((video, idx) => {
      const label = videoLinks.length > 1 ? `Tonton video ${idx + 1}` : 'Tonton video';
      return `<a href="${video}" target="_blank" rel="noopener noreferrer">${label} &nbsp;▶</a>`;
    }).join('');
    const actionHtml = [linksHtml, videosHtml].filter(Boolean).join('');
    return `<article class="material"><div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px"><span class="tag">${mapelNames[m.mapel] || m.mapel}</span>${sourceBadge}</div><h3>${m.judul}</h3><p>${m.deskripsi}</p><div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center">${actionHtml}</div>${adminActions}</article>`; 
...
function getMaterialLinkInputs() {
  ...
  document.querySelectorAll('#editor-link, .material-link-input').forEach(input => {
    if (input instanceof HTMLInputElement) {
      pushUnique(input.value);
    }
  });
...
function resetMaterialLinkInputs(links = []) {
  ...
  const legacyInputs = document.querySelectorAll('#editor-link:not(.material-link-input)');
...
function openMaterialEditor(index = null) {
  ...
  const videoField = document.getElementById('editor-video');
  if (videoField) videoField.value = material.video || '';
...
async function saveMaterial() {
  ...
  const video = document.getElementById('editor-video')?.value.trim() || '';
  const mapel = document.getElementById('editor-mapel').value;
  
  // Validate input
  if (!validateMaterial(judul, deskripsi, links, video)) return;
  
  // Set loading state pada tombol simpan materi saja agar tidak tertukar dengan tombol simpan lain.
  const saveBtn = document.getElementById('material-save-button') || document.querySelector('#material-editor .save-button');
  ...
  try {
    const material = {
      ...
      judul,
      deskripsi,
      link: links.length > 1 ? links : links[0],
